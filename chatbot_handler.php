...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['user_input'])) {
    ...
    $lowercase_input = strtolower($user_input);
    foreach ($predefined_responses as $question => $answer) {
        if (strpos($lowercase_input, $question) !== false) {
    ...
    $context = "Tu es un assistant pour Green.tn, un site web dédié à la mobilité verte, notamment les vélos et les services associés. Réponds aux questions des utilisateurs concernant les réclamations, les avis, ou les services de Green.tn. Ne parle pas de voitures ou d'autres sujets non pertinents.";
    ...
    $result = curl_exec($ch);
    $http_code = curl_getinfo($ch, CURLINFO_HTTP_CODE); // Obtenir le code de statut HTTP
    curl_close($ch);

    if ($result === false) {
        error_log("cURL Error: " . curl_error($ch) . " (HTTP Code: $http_code)");
        echo json_encode(['response' => "Désolé, une erreur s'est produite avec l'API. Essayez à nouveau ou contactez le support."]);
        exit;
    }
    ...
    if (isset($response_data[0]['generated_text'])) {
        ...
        error_log("API Error: " . $response_data['error'] . " (HTTP Code: $http_code)");
        echo json_encode(['response' => "Désolé, une erreur s'est produite avec l'API. Essayez à nouveau ou contactez le support."]);
    } else {
        error_log("Unexpected API response: " . $result . " (HTTP Code: $http_code)");
        echo json_encode(['response' => "Désolé, une erreur s'est produite. Essayez à nouveau."]);
    }
} else {
