...
if ($isLoggedIn) {
    try {
        $query = "SELECT role FROM users WHERE id = :user_id"; // Uniformisé sur 'users'
    ...
    try {
        ...
        if ($avis && $avis['user_id'] == $_SESSION['user_id']) {
            ...
            if ($stmt->execute()) {
                ...
                header("Location: " . $_SERVER['PHP_SELF']); // Rediriger pour éviter re-soumission
                exit();
            } else {
...
if ($isLoggedIn) {
    try {
        ...
        $total_pages = ceil($total_avis / $avis_per_page);
    } catch (PDOException $e) {
        $message = t('database_error');
    }
}
...
<head>
    ...
    <style>
        /* (Styles inchangés, préservés tels quels) */
        * {
            margin: 0;
            padding: 0;
...
<body>
    <header>
        <div class="logo-nav-container">
            ...
            <nav class="nav-left">
                <ul>
                    ...
                    <?php if ($isAdmin): ?>
                        <li><a href="../../VIEW/reclamation/liste_avis.php"><?php echo t('view_reviews'); ?></a></li>
                    <?php endif; ?>
                </ul>
            </nav>
        </div>
        <nav class="nav-right">
            <ul>
                ...
                <?php endif; ?>
                <li>
                    <form action="../../VIEW/reclamation/mes_avis.php" method="POST">
                ...
                <?php else: ?>
                    <?php foreach ($avis as $avi): ?>
                        <div class="avis">
                            ...
                            <form method="POST" action="../../VIEW/reclamation/mes_avis.php" style="display:inline;" onsubmit="return confirm('<?php echo t('confirm_delete'); ?>');">
                    ...
                    <div class="pagination">
                        <?php if ($page > 1): ?>
                            <a href="../../VIEW/reclamation/mes_avis.php?page=<?php echo $page - 1; ?>"><?php echo t('previous'); ?></a>
                        <?php else: ?>
                            <a href="#" class="disabled"><?php echo t('previous'); ?></a>
                        <?php endif; ?>

                        <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                            <a href="../../VIEW/reclamation/mes_avis.php?page=<?php echo $i; ?>" class="<?php echo $i == $page ? 'current' : ''; ?>"><?php echo $i; ?></a>
                        <?php endfor; ?>

                        <?php if ($page < $total_pages): ?>
                            <a href="../../VIEW/reclamation/mes_avis.php?page=<?php echo $page + 1; ?>"><?php echo t('next'); ?></a>
                        <?php else: ?>
    ...
    <script>
        // Ajouter une confirmation pour la suppression (déjà gérée dans le formulaire)
    </script>
</body>
</html>