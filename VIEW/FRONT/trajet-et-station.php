...
<head>
    ...
    <link rel="stylesheet" href="../FRONT/assets/public/green.css">
    <link rel="stylesheet" href="../FRONT/assets/trajet-et-station.css">
...
<div class="hamburger-menu">
    ...
    <main>
        ...
        <section class="container">
            <section class="illustration">
                <img src="../FRONT/image/lost.png" alt="Illustration">
            </section>
    ...
    <script>
        function openRouteInMaps(button) {
            try {
                const startLocation = button.getAttribute('data-start');
                const endLocation = button.getAttribute('data-end');
                const startName = button.getAttribute('data-start-name');
                const endName = button.getAttribute('data-end-name');
                const routeCoordinates = JSON.parse(button.getAttribute('data-route'));
                ...
                L.marker([startLat, startLng])
                    .addTo(routeLayer)
                    .bindPopup(`<b>Départ:</b> ${startName}`);

                // Add end marker
                const [endLat, endLng] = endLocation.split(',').map(Number);
                L.marker([endLat, endLng])
                    .addTo(routeLayer)
                    .bindPopup(`<b>Arrivée:</b> ${endName}`);
                ...
                document.getElementById('route-details').innerHTML = `
                    <div class="route-info">
                        ...
                        <p><strong>Départ:</strong> ${startName}</p>
                        <p><strong>Arrivée:</strong> ${endName}</p>
                    </div>
                `;

                // Update OSM button
                document.getElementById('open-osm').onclick = function() {
                    const url = `https://www.openstreetmap.org/directions?engine=fossgis_osrm_bike&route=${startLat},${startLng};${endLat},${endLng}`;
                    window.open(url, '_blank');
                };
                ...
                console.error("Error opening route in maps:", error);
                alert("Une erreur est survenue lors de l'affichage de la carte : " + error.message);
            }
        }
    </script>
</body>
