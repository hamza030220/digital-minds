...
try {
    ...
    $valid_sort_columns = ['id_velo', 'nom_velo', 'prix_par_jour', 'disponibilite', 'deleted_at'];
    if (!in_array($sort_by, $valid_sort_columns)) {
        $sort_by = 'id_velo';
    }

    // Compter le nombre total de vélos dans la corbeille
    $sql_count = "SELECT COUNT(*) FROM corbeille_velos WHERE nom_velo LIKE :search OR type_velo LIKE :search";
    $stmt_count = $pdo->prepare($sql_count);
    $stmt_count->execute(['search' => "%$search%"]);
    $total_velos = $stmt_count->fetchColumn();
    $total_pages = ceil($total_velos / $velos_per_page);

    // Récupérer les vélos dans la corbeille
    $sql = "SELECT * FROM corbeille_velos 
    ...
    if (isset($_GET['action']) && isset($_GET['id']) && is_numeric($_GET['id'])) {
        ...
        if ($action === 'restore') {
            // Récupérer le vélo
            $stmt = $pdo->prepare("SELECT * FROM corbeille_velos WHERE id_velo = ?");
            $stmt->execute([$id]);
            $velo = $stmt->fetch(PDO::FETCH_ASSOC);
            
            if ($velo) {
                // Restaurer dans velos
                $stmt = $pdo->prepare("INSERT INTO velos (id_velo, nom_velo, type_velo, prix_par_jour, disponibilite) 
                                       VALUES (?, ?, ?, ?, ?)");
                $stmt->execute([
                    $velo['id_velo'],
                    $velo['nom_velo'],
                ...
                // Supprimer de corbeille_velos
                $stmt = $pdo->prepare("DELETE FROM corbeille_velos WHERE id_velo = ?");
                $stmt->execute([$id]);
            }
        ...
        header("Location: corbeille.php?page=$current_page&search=" . urlencode($search) . "&sort_by=$sort_by");
        exit();
    }
...
<head>
    ...
    <title>Corbeille des Vélos - Green.tn</title>
...
<body>
    ...
    <div class="main-content" id="main">
        ...
        <div class="section-content">
            ...
            <div class="task-bar">
                <form method="get" class="search-container">
                    ...
                    <select name="sort_by" id="sort_by" class="search-input" onchange="this.form.submit()">
                        ...
                        <option value="deleted_at" <?php echo $sort_by == 'deleted_at' ? 'selected' : ''; ?> data-translate="deleted_at">Date de suppression</option>
                    </select>
            ...
            <!-- Table -->
            <a href="velos.php?super_admin=1" class="btn" style="margin-bottom: 20px;"><i class="bi bi-arrow-left"></i> Retour aux vélos</a>
            <table class="velo-table">
                <thead>
                    <tr>
                        ...
                        <th><i class="bi bi-clock"></i> <span data-translate="deleted_at">Date de suppression</span></th>
                ...
                <tbody>
                    <?php if (empty($velos)): ?>
                        <tr>
                            <td colspan="7" style="text-align: center;">Aucun vélo dans la corbeille.</td>
                        </tr>
                    <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="?action=restore&id=<?php echo $velo['id_velo']; ?>&page=<?php echo $current_page; ?>&search=<?php echo urlencode($search); ?>&sort_by=<?php echo $sort_by; ?>" 
                                   class="btn restore" 
                                   title="Restaurer" 
                                   onclick="return confirm('Êtes-vous sûr de vouloir restaurer ce vélo ?');">
                                    <i class="bi bi-arrow-counterclockwise"></i>
                                </a>
                                <a href="?action=delete&id=<?php echo $velo['id_velo']; ?>&page=<?php echo $current_page; ?>&search=<?php echo urlencode($search); ?>&sort_by=<?php echo $sort_by; ?>" 
                                   class="btn delete" 
                                   title="Supprimer définitivement" 
                                   onclick="return confirm('Êtes-vous sûr de vouloir supprimer définitivement ce vélo ?');">
            ...
            <div class="pagination">
                <a href="?page=<?php echo $current_page - 1; ?>&search=<?php echo urlencode($search); ?>&sort_by=<?php echo $sort_by; ?>" 
                   class="btn <?php echo $current_page <= 1 ? 'disabled' : ''; ?>" 
                   title="Page précédente">
                    <i class="bi bi-arrow-left"></i>
                </a>
                <a href="?page=<?php echo $current_page + 1; ?>&search=<?php echo urlencode($search); ?>&sort_by=<?php echo $sort_by; ?>" 
                   class="btn <?php echo $current_page >= $total_pages ? 'disabled' : ''; ?>" 
                   title="Page suivante">
