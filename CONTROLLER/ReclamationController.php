...
class ReclamationController {
    ...
    public function handlePostRequest() {
        ...
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            ...
            header('Location: ../../projetweb/VIEW/reclamation/ajouter_reclamation.php');
            exit;
        }

        // Ensure user is logged in and not an admin to process POST
        if (!$this->isLoggedIn) {
        ...
        if ($this->isAdmin) {
            // Store error message for admin access
            $_SESSION['flash_message'] = "Accès réservé aux utilisateurs non-admin.";
            $_SESSION['flash_message_type'] = 'error';
            // Redirect back to the form page
            header('Location: ../../projetweb/VIEW/reclamation/ajouter_reclamation.php');
            exit;
